    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<InvoiceModel>>(
        ...
        builder: (context, snap) {
          ...
          if (invoices.isEmpty) return const Center(child: Text('No paid bills yet.'));
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                title: Text('Invoice ${inv.invoiceID} • RM ${inv.amount.toStringAsFixed(2)}'),
                subtitle: Text('${inv.plateNumber} • ${_ymd(inv.date)}'),
                trailing: const Icon(Icons.chevron_right),
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => FeedbackDetailPage(invoice: inv)),
                ),
              );
            },
          );
...
class _FeedbackDetailPageState extends State<FeedbackDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (loading) {
      return Scaffold(appBar: AppBar(title: const Text('Feedback')), body: const Center(child: CircularProgressIndicator()));
    }
        ...
        child: Column(
          children: [
            ...
            Align(
              ...
              child: Text('Your Feedback', style: Theme.of(context).textTheme.titleMedium),
            ),
            if (!hasExisting)
              const Padding(
                padding: EdgeInsets.only(top: 6, bottom: 6),
                child: Align(alignment: Alignment.centerLeft, child: Text('No feedback yet.')),
              ),
  ...
  Future<void> _pickAndUpload(ImageSource source) async {
    try {
      ...
      final uid = FirebaseAuth.instance.currentUser!.uid;
      final path = 'feedback_photos/$uid/${widget.invoice.invoiceID}-${DateTime.now().millisecondsSinceEpoch}.jpg';
